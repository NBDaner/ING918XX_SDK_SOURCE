...
int32_t sbc_enc_init(sbc_encoder* sbc, int32_t sample_rate, int32_t num_channels)
{
    ...
    {
    case 1:
        frame->channel_mode = SBC_CHANNEL_MODE_MONO;
        break;
    case 2:
        frame->channel_mode = SBC_CHANNEL_MODE_DUAL_CHANNEL;
        break;
    case 3:
        frame->channel_mode = SBC_CHANNEL_MODE_JOINT_STEREO;
        break;
    default:
...
uint32_t sbc_enc_get_codesize(sbc_encoder *sbc)
{
	...
	subbands = sbc->frame.subbands;
	blocks = sbc->frame.blocks;
	channels = sbc->frame.channel_mode == SBC_CHANNEL_MODE_MONO ? 1 : 2;
...
uint32_t sbc_enc_get_frame_length(sbc_encoder *sbc)
{
	...
	subbands = sbc->frame.subbands ? 8 : 4;
	blocks = sbc->frame.blocks;
	channels = sbc->frame.channel_mode == SBC_CHANNEL_MODE_MONO ? 1 : 2;
	joint = sbc->frame.channel_mode == SBC_CHANNEL_MODE_STEREO ? 1 : 0;
	bitpool = sbc->frame.bitpool;

	ret = 4 + (4 * subbands * channels) / 8;
	/* This term is not always evenly divide so we round it up */
	if (channels == 1 || sbc->frame.channel_mode == SBC_CHANNEL_MODE_DUAL_CHANNEL)
